...
final="Thomas-Sun_few-HELM-FagB_few-Sun-FagB-questions-explain--questions-rate--nuggets-explain--nuggets-rate--all-benchmarkY3test-qrels-runs-with-text.jsonl.gz"
echo "Graded: $final"
...
#
# For CAR, where the judgment 1 is a non-relevant grade, the default can be used (default is 1)
# Since the run files use query/heading, we need to enable --qrel-query-facets
#
...
# car-autograde-inter-annotator-\$promptclass.tex:  LaTeX tables with graded and binarized inter-annotator statistics with Cohen's kappa agreement. ``Min-anwers'' refers to the number of correct answers obtained above a self-rating threshold by a passage. (For \dl{} –-min-relevant-judgment 2 must be set.)
# 
...
python -O -m exam_pp.exam_leaderboard_analysis data/car/$final  --question-set question-bank --prompt-class  QuestionSelfRatedUnanswerablePromptWithChoices NuggetSelfRatedPrompt Thomas FagB FagB_few HELM Sun Sun_few --min-relevant-judgment 1 --trec-eval-metric ndcg_cut.10 ndcg_cut.20 map Rprec recip_rank  --use-ratings --qrel-dir=data/car --qrel-analysis-out data/car/car-autograde-qrels-leaderboard-analysis-graded.correlation.tsv --run-dir data/car/carruns --official-leaderboard data/car/official_car_leaderboard.json --qrel-query-facets  --question-set-for-facets question-bank --question-path-for-facets data/car/car-questions.jsonl.gz --cover-analysis-out data/car/car-autograde-cover-leaderboard-analysis-graded.correlation.tsv
...
for promptclass in  QuestionSelfRatedUnanswerablePromptWithChoices NuggetSelfRatedPrompt Thomas; do
        ...
        for minrating in 4 5; do
                # autograde-qrels
                # qrel leaderboard correlation
                # N.B. requires TREC-CAR-Y3 runs to be populated in data/car/carruns
                # python -O -m exam_pp.exam_post_pipeline data/car/$final  --question-set question-bank --prompt-class $promptclass  --min-relevant-judgment 1 --trec-eval-metric $eval_metric  --use-ratings --min-trec-eval-level ${minrating} -q data/car/car-exam-$promptclass.qrel --qrel-leaderboard-out data/car/car-autograde-qrels-leaderboard-$promptclass-$eval_metric-minlevel-$minrating.correlation.tsv --run-dir data/car/carruns --official-leaderboard data/car/official_car_leaderboard.json --qrel-query-facets  --question-set-for-facets question-bank --question-path-for-facets data/car/car-questions.jsonl.gz
        
                # autograde-cover 
                #python -O -m exam_pp.exam_post_pipeline data/car/$final  --question-set question-bank --prompt-class $promptclass  --min-relevant-judgment 1 --use-ratings --min-self-rating ${minrating} --leaderboard-out data/car/car-autograde-cover-leaderboard-$promptclass-minlevel-$minrating.correlation.tsv  --official-leaderboard data/car/official_car_leaderboard.json --qrel-query-facets  --question-set-for-facets question-bank --question-path-for-facets data/car/car-questions.jsonl.gz
                echo ""
        done
        ...
        python -O -m exam_pp.exam_post_pipeline data/car/$final  --question-set question-bank --prompt-class $promptclass  --min-relevant-judgment 1 --use-ratings  --inter-annotator-out data/car/car-autograde-inter-annotator-$promptclass.tex --qrel-query-facets --qrel-query-facets  --question-path data/car/car-questions.jsonl.gz
done
...
for promptclass in  FagB FagB_few HELM Sun Sun_few; do
        ...
        #python -O -m exam_pp.exam_post_pipeline data/car/$final  --question-set question-bank --prompt-class $promptclass  --min-relevant-judgment 1 --min-trec-eval-level 1 --trec-eval-metric $eval_metric -q data/car/car-exam-$promptclass.qrel --qrel-leaderboard-out data/car/car-autograde-qrels-leaderboard-$promptclass-$eval_metric.correlation.tsv --run-dir data/car/carruns --official-leaderboard data/car/official_car_leaderboard.json --qrel-query-facets  --question-path data/car/car-questions.jsonl.gz
        
        # autograde-cover 
        #python -O -m exam_pp.exam_post_pipeline data/car/$final  --question-set question-bank --prompt-class $promptclass  --min-relevant-judgment 1 --leaderboard-out data/car/car-autograde-cover-leaderboard-$promptclass.correlation.tsv  --official-leaderboard data/car/official_car_leaderboard.json --qrel-query-facets  --question-path data/car/car-questions.jsonl.gz
                echo ""

        python -O -m exam_pp.exam_post_pipeline data/car/$final  --question-set question-bank --prompt-class $promptclass  --min-relevant-judgment 1 --inter-annotator-out data/car/car-autograde-inter-annotator-$promptclass.tex --qrel-query-facets  --question-path data/car/car-questions.jsonl.gz
